...
const SingleCocktail = () => {
  ...
  const newIngredients = ingredients.filter((item) => item != null)
  return (
        ...
        <div className="drink-info">
          ...
          <p>
            ...
            {newIngredients.map((item, index) => {
              // console.log("lenght: " + newIngredients.length);
              // console.log(newIngredients);
              return item ? <span key={index}>{item} {`${((newIngredients.length - 1) !== index && item != null) ? "," : ''}`}</span> : null
            })}
          </p>
